...
void Fish::init(Fish::type type, sf::Texture background) {
	//setting working area
	work_area.x = background.getSize().x * 3 + 100;
	work_area.y = background.getSize().y * 2;
...
void Fish::update(float dt) {
	...
	if (returning_frame > 0)
		returning();
	else if (Eating_frame > 0)
		eating();
	else if (Eated_frame > 0)
		eated();
	else
		swimming();
	...
	if (!escaping) {
		...
		if (change_direction_timer >= rand() % 10 + 10) {
			if (!X_Y_direction.x) {
				if (sprite.getPosition().x >= work_area.x) {
					X_Y_direction.x = -1;
				}
	...
	sprite.move(
		X_Y_direction.x * int(current_type) * freeze * std::cos(angle) * speed * deltatime,
		X_Y_direction.x * std::sin(angle) * X_Y_direction.y * speed * deltatime * int(current_type) * freeze
	);
...
void Fish::returning() {
	...
	if (timer >= 0.08f) {
		if (returning_frame > 0) {
			...
			case Fish::type::MINNOW:
				fish_row = 1;
				fish_column = 6 - returning_frame + 1;
				fish_widthSize = 991 / 15.0f;
				fish_heightSize = 99 / 2.0f;
				sprite.setTextureRect(sf::IntRect(fish_column * fish_widthSize + 2,
			...
			case Fish::type::SURGEON_FISH:
				fish_column = 5 - returning_frame + 1;
				fish_row = 2;
				fish_widthSize = 2395 / 14.0f;
				fish_heightSize = 319 / 3.0f;
				sprite.setTextureRect(sf::IntRect(fish_column * fish_widthSize + 2,
...
void Fish::isReturning() {
	...
	case Fish::type::SURGEON_FISH:
	case Fish::type::LION_FISH: returning_frame = 6; break;
	}
	Eating_frame = 0;
}
